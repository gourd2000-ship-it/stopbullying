...
## 🗺️ DB 연동 로드맵
- 현재 이 프로젝트의 상담 기록은 개별 사용자의 브라우저 로컬 스토리지(`localStorage`)에 저장되므로 다른 기기의 관리자 화면에서는 볼 수 없습니다.
- 다수의 기기에서 수집되는 상담 기록을 실시간 데이터베이스(DB)로 안전하게 연동하는 방법은 [DB 연동 로드맵](docs/db_integration_roadmap.md) 문서를 참고해 주세요.
